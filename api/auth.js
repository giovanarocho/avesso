...
// sessão) ou body.action (POST, pro resto).
...
async function actionLogin(req, res, body) {
  if (!contaAuthConfigured()) {
    res.status(500).json({ error: 'CONTA_SESSION_SECRET não configurado no servidor' });
    return;
  }
  ...
  const token = issueContaToken(email);
...
async function actionSetPassword(req, res, body) {
  ...
  if (!dbConfigured()) {
    res.status(500).json({ error: 'banco não configurado — contas precisam do supabase' });
    return;
  }
  ...
  // confere no mercado pago, ao vivo, que esse pagamento é aprovado e é de
  // algum produto do catálogo — nunca confia em nada que venha só do
  // navegador. mesmo princípio anti-fraude de sempre.
  ...
  // o e-mail da conta tem que ser o mesmo que pagou — confere contra o que
  // ficou salvo no banco na hora da criação do pix.
  ...
  const created = await dbInsertUsuario({ email, password_hash: hash, password_salt: salt });
  if (!created) {
...
async function actionRequestReset(req, res, body) {
  ...
  if (user) {
    ...
    const resetLink = (SITE_URL || '') + '/conta?reset=1&token=' + encodeURIComponent(token) + '&email=' + encodeURIComponent(email);
...
async function actionResetPassword(req, res, body) {
  ...
  const { hash, salt } = hashPassword(password);
  const ok = await dbUpdateSenha(email, hash, salt);
  if (!ok) {
    res.status(500).json({ error: 'não consegui salvar a nova senha agora' });
    return;
  }
  const sessionToken = issueContaToken(email);
