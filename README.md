...
- `shared/venda.css` — o visual padrão das páginas de venda (`/guia-venda` e `/molda` usam o mesmo). Uma ferramenta nova só precisa linkar esse arquivo e usar as classes prontas pra sair com a mesma cara, sem escrever CSS do zero.
...
3. (opcional) Sobe uma página de venda própria pra ela, no mesmo padrão visual das outras: inclui `<link rel="stylesheet" href="/shared/venda.css">` e monta a página com as classes prontas (`.hero`, `.trust-row`, `.publico` pra "pra quem é / não é", `.passos` ou `.features` pro que a pessoa recebe, `.como`, `.faq`, `.cta-final`) — copia `molda/index.html` inteiro como ponto de partida, já que ele só usa classes do `venda.css` (sem estilo próprio). O checkout reaproveita com `<script src="/shared/checkout.js" data-produto="seu-slug"></script>` e o html do modal (também já em `molda/index.html`). Isso importa porque essas páginas costumam circular sozinhas — divulgadas por afiliados sem ninguém ter passado pela home antes — então elas têm que se sustentar sem contexto nenhum do resto do site.
